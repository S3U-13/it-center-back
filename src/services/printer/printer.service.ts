...
export class PrinterService {
  static async index(query: Record<string, any>) {
    ...
    const { rows, count } = await db.Printer.findAndCountAll({
      where,
  ...
  static async create(body: any) {
    ...
    if (!array_data || !Array.isArray(array_data) || array_data.length === 0) {
      throw new Error("Printer must be a non-empty array");
    }
    ...
    //const map
    const mappingPrinter = array_data.map((i, index) => {
      if (!i) {
    ...
    return await db.Printer.bulkCreate(mappingPrinter);
  }
  static async show(id: number) {
    return await db.Printer.findByPk(id, {
      // attributes: [
      //   "sn",
      //   "rpj_no",
      //   "note",
