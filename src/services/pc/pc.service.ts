...
export class PcService {
  static async index(query: Record<string, any>) {
    ...
    const { rows, count } = await db.Pc.findAndCountAll({
      where,
  ...
  static async create(body: any) {
    ...
    if (!array_data || !Array.isArray(array_data) || array_data.length === 0) {
      throw new Error("Pc must be a non-empty array");
    }
    ...
    //const map
    const mappingPc = array_data.map((i, index) => {
      if (!i) {
    ...
    return await db.Pc.bulkCreate(mappingPc);
  }
  static async show(id: number) {
    return await db.Pc.findByPk(id, {
      // attributes: [
      //   "sn",
      //   "rpj_no",
      //   "note",
  ...
  static async edit(id: number, body: any) {
    ...
    const Pc = await db.Pc.findByPk(id);
